...
const products = [
  ...
  {
    id: 5,
    title: 'Breezy Linen Shirt',
    category: 'Casual Summer',
    price: '$55',
    rating: 4.5,
    reviews: 96,
    image: 'https://storage.googleapis.com/workspace-0f70711f-8b4e-4d94-86f1-2a93ccde5887/image/92865840-2085-4577-9816-b0a86f16aa3d.png',
    badge: 'SUMMER COLLECTIONS',
...
const Collection = () => {
  ...
  const [filter, setFilter] = useState<
    'ALL' | 'BEST SELLER' | 'NEW ARRIVAL' | 'MENS' | 'WOMEN' | 'SUMMER COLLECTIONS'
  >('ALL');

  const filteredProducts =
    filter === 'ALL' ? products : products.filter((product) => product.badge === filter);
    ...
    <section className="px-5 lg:px-28 py-24 bg-gradient-to-b from-[#F9FAFB] to-[#F1F5F9] text-[#1F2937]">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl font-extrabold text-[#111827] pb-5">Collections</h2>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          {['ALL', 'NEW ARRIVAL', 'BEST SELLER', 'MENS', 'WOMEN', 'SUMMER COLLECTIONS'].map((btn) => (
            <button
              key={btn}
              onClick={() => setFilter(btn as typeof filter)}
              className={`px-6 py-2.5 rounded-full font-semibold transition duration-200 text-sm border ${
                filter === btn
            ...
            >
              ...
              <motion.button
                className={`absolute bottom-0 left-0 right-0 text-center py-3 font-semibold text-white bg-[#1F2937] transition-all duration-300 ${
                  hoveredProduct === product.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full'
                }`}
